...
from info import FILE_AUTO_DEL_TIMER, UPI_PAY_LOGS, ADMINS, THREE_VERIFY_GAP, LOG_CHANNEL, USERNAME, VERIFY_IMG, IS_VERIFY, FILE_CAPTION, AUTH_CHANNEL, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, LOG_API_CHANNEL, TWO_VERIFY_GAP, QR_CODE, DELETE_TIME
...
async def force_subscribe(client, message):
    ...
    try:
        ...
        logger.exception(e)
        return await m.edit(f"Something went wrong ! Try again later or report in Support Group @tamilmovierequestda")
...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /set_stream_link link")
    except IndexError:
        await message.reply("Usage: /set_stream_link link")
        return
